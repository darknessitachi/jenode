...
public class PluginUI extends UIFacade {
	...
	public void bindTree(TreeAction ta) {
		...
		for (PluginConfig pc : PluginManager.getInstance().getAllPluginConfig()) {
			dt.insertRow(new Object[] { pc.getID(),
					pc.getName(User.getLanguage()), pc.getAuthor(),
					pc.getVersion() });
		}
		ta.bindData(dt);
		...
		for (int i = 1; i < ta.getItemSize(); i++) {
			...
			if (!PluginManager.getInstance()
					.getPluginConfig(ta.getItem(i).getData().getString("ID"))
					.isEnabling())
	...
	public void bindPluginGrid(DataGridAction dga) {
		...
		if (pc != null) {
			...
			for (String id : pc.getRequiredPlugins().keyArray()) {
				PluginConfig pc2 = PluginManager.getInstance().getPluginConfig(
						id);
				dt.insertRow(new Object[] { pc2.getID(),
						pc2.getName(User.getLanguage()), pc2.getVersion() });
			}
			dga.bindData(dt);
		}
	}
	...
	public void bindDependOnPluginGrid(DataGridAction dga) {
		PluginConfig pc = PluginManager.getInstance().getPluginConfig($V("ID"));
		Mapx<String, PluginConfig> map = new Mapx();
		if (pc != null) {
	...
	private void addDependOn(Mapx<String, PluginConfig> map,
			ArrayList<PluginConfig> list, String id) {
	...
	public void bindExtendPointGrid(DataGridAction dga) {
		...
		for (ExtendPointConfig ep : pc.getExtendPoints().valueArray()) {
			dt.insertRow(new Object[] { ep.getID(),
					"platform/images/plugin_point.gif", ep.getClassName(),
					Boolean.valueOf(ep.getUIFlag()),
	...
	public void bindExtendActionGrid(DataGridAction dga) {
		...
		for (ExtendActionConfig ep : pc.getExtendActions().valueArray()) {
			dt.insertRow(new Object[] { ep.getID(),
					"platform/images/plugin_action.gif", ep.getClassName(),
					ep.getExtendPointID(),
					ep.getDescription(User.getLanguage()) });
		}
		dga.bindData(dt);
	...
	public void bindMenuGrid(DataGridAction dga) {
		...
		for (Menu menu : pc.getMenus().valueArray())
			if ("Backend".equals(menu.getType())) {
				dt.insertRow(new Object[] { menu.getID(), menu.getParentID(),
						menu.getName(User.getLanguage()), menu.getIcon(),
						menu.getURL(), menu.getOrder(),
						menu.getDescription(User.getLanguage()),
						menu.getPluginConfig().getID(),
	...
	private void setParentAndName(DataTable dt, Mapx<String, String> map,
			String path) {
		if (map.containsKey(path)) {
			return;
		}
		dt.insertRow((DataRow)null);
	...
	public void setStatus() {
		...
		try {
			if ("true".equals(status)) {
				if (pc.isEnabling()) {
					fail(failed + ":"
							+ Lang.get("Platform.PluginAlreadyEnable"));
					return;
				}
				...
				if (!pc.isEnabling()) {
					fail(failed + ":"
							+ Lang.get("Platform.PluginAlreadyDisabled"));
					return;
				}
