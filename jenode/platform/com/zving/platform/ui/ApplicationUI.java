...
public class ApplicationUI extends UIFacade
{
  ...
  public void init()
  {
    $S("Privs", StringUtil.javaEncode(UserBL.getUserPriv(User.getUserName()).toString()));
  ...
  private static String getImgSpirite(Object oImgSrc)
  {
    ...
    Matcher matcher = Pattern.compile("[^\"]*icons\\/([^\"\\/]+)\\.png", 34).matcher(imgSrc);
    String imgTag;
    String imgTag;
    if (matcher.find()) {
  ...
  public void changeLanguage() {
    ...
    Current.getCookies().setCookie("_ZVING_LANGUAGE", lang, Config.getContextPath());
  ...
  public void changeLoginAccount(ZAction za)
  {
    ...
    LogBL.addLog(agentUser, "UserLog", "Entrust", "User " + agentUser + " is now agent of " + userName);
  ...
  public void bindEntrustUsers(DataGridAction dga)
  {
    ...
    if (list != null) {
      ...
      dt = new Q("select ZDUser.*,(select Name from ZDBranch where BranchInnerCode=ZDUser.BranchInnerCode) as BranchName from ZDUser where UserName in ('" + 
        userNames + "')", new Object[0])
        .fetch();
    }
