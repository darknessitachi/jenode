...
public class CodeService extends AbstractExtendService<FixedCodeType> {
	...
	public static void init() {
		...
		for (ZDCode code : new ZDCode().query())
			dbSet.add("@CodeType=" + code.getCodeType() + "@ParentCode="
					+ code.getParentCode() + "@CodeValue="
					+ code.getCodeValue());
		...
		for (FixedCodeType fct : getInstance().getAll()) {
			...
			if (!dbSet.contains("@CodeType=" + fct.getCodeType()
					+ "@ParentCode=System@CodeValue=System")) {
			...
			localIterator2 = items.iterator();

			for (FixedCodeType.FixedCodeItem item : items) {

				if (!dbSet.contains("@CodeType=" + code.getCodeType()
						+ "@ParentCode=" + code.getCodeType() + "@CodeValue="
						+ item.getValue())) {
