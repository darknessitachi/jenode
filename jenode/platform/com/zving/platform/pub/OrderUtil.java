...
public class OrderUtil {
	...
	public static void updateOrder(String table, String column,
			long targetOrderFlag, String wherePart, DAO<?> sortDAO,
			Transaction tran) {
		Mapx mapx = sortDAO.toMapx();
		long orderFlag = mapx.getLong(column);
		long newOrderFlag = targetOrderFlag;
		if (orderFlag > targetOrderFlag) {
			Q qb1 = new Q("update " + table + " set " + column + "=" + column
					+ "+1 where " + column + ">=? and " + column + "<? "
					+ wherePart, new Object[] { Long.valueOf(targetOrderFlag),
					Long.valueOf(orderFlag) });
			tran.add(qb1);
		} else {
			Q qb1 = new Q("update " + table + " set " + column + "=" + column
					+ "-1 where " + column + ">? and " + column + "<=? "
					+ wherePart, new Object[] { Long.valueOf(orderFlag),
					Long.valueOf(targetOrderFlag) });
			tran.add(qb1);
		}
	...
	public static void updateOrder(String table, String column,
			long targetOrderFlag, DAO<?> sortDAO, Transaction tran) {
		updateOrder(table, column, targetOrderFlag, "", sortDAO, tran);
	}

	public static void updateOrder(String table, long targetOrderFlag,
			String wherePart, DAO<?> sortDAO, Transaction tran) {
		updateOrder(table, "OrderFlag", targetOrderFlag, wherePart, sortDAO,
				tran);
	}

	public static void updateOrder(String table, long targetOrderFlag,
			DAO<?> sortDAO, Transaction tran) {
		...
		for (int i = 0; i < arr.length; i++) {
			if (arr[i] == target) {
				if (flag) {
						...
						if (j != i) {
							...
							qb = new Q(
									"update " + table + " set " + column
											+ "=?, ModifyTime=? where "
											+ column + "=?",
									new Object[] { Long.valueOf(target * 10L) });
						}
					...
					for (int j = 0; j < i; j++) {
						if (arr[j] + 1L != arr[(j + 1)]) {
							qb = new Q("update " + table + " set " + column
									+ "=" + column + "-?, ModifyTime=? where "
									+ column + " between ? and ? and "
									+ wherePart, new Object[0]);
					...
					for (int j = arr.length - 1; j > i; j--)
						if (arr[j] != arr[(j - 1)] + 1L) {
							...
							qb.add(new Object[] { Integer.valueOf(arr.length
									- j) });
							qb.add(new Object[] { modifyTime });
							qb.add(new Object[] { Long.valueOf(arr[(j - 1)]) });
							qb.add(new Object[] { Long.valueOf(arr[j]) });
				...
				qb = new Q("update " + table + " set " + column + "=" + column
						+ "/10 where " + column + ">? and " + wherePart,
						new Object[] { Long.valueOf(target * 9L) });
				tran.add(qb);
				if (bFlag) {
					return true;
				}
