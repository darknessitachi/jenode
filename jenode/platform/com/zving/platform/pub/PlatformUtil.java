...
public class PlatformUtil {
	...
	public static String getFileIcon(String path) {
		...
		if (path.indexOf(".") > 0) {
			...
			if (ObjectUtil.in(new Object[] { ext, "", "asp", "aspx", "avi",
					"bmp", "doc", "docx", "exe", "fla", "flv", "folder", "gif",
					"html", "jpg", "js", "jsp", "mdb", "mov", "mp3", "mp4",
					"pdf", "php", "png", "ppt", "rar", "rm", "swf", "txt",
					"wmp", "wmv", "xls", "zip" })) {
	...
	public static String getFileIconBySuffix(String suffix) {
		...
		if (ObjectUtil.in(new Object[] { ext, "", "asp", "aspx", "avi", "bmp",
				"doc", "docx", "exe", "fla", "flv", "folder", "gif", "html",
				"jpg", "js", "jsp", "mdb", "mov", "mp3", "mp4", "pdf", "php",
				"png", "ppt", "rar", "rm", "swf", "txt", "wmp", "wmv", "xls",
				"zip" })) {
			return "framework/images/filetype/" + ext + ".gif";
		}
		return "framework/images/filetype/unknown.gif";
	...
	public static void initDistrict() {
		District = new Q("select Name,Code,TreeLevel,Type from zddistrict",
				new Object[0]).fetch();
		Mapx map = new Mapx();
	...
	public static void loadDBConfig() {
		synchronized (mutex) {
			if ((Config.getMapx().containsKey("Database.Default.Type"))
					&& ("true".equals(Config.getMapx().get(
							"App.ExistPlatformDB"))))
				try {
					DataTable dt = new Q("select code,value from zdconfig",
							new Object[0]).fetch();
					for (int i = 0; (dt != null) && (i < dt.getRowCount()); i++) {
						Config.getMapx().put(dt.getString(i, 0),
								dt.getString(i, 1));
					}
					Config.getMapx().put("AdminUserName",
