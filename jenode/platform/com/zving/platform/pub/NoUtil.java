...
public class NoUtil
{
  ...
  public static synchronized long getMaxIDUseLock(String noType, String subType)
  {
    ...
    try {
      ...
      Q qb = new Q("select NoMaxValue from ZDMaxNo where NoType=? and NoSubType=?", new Object[] { noType, subType });
      if (Config.isOracle()) {
      ...
      if (maxValue != null) {
        ...
        qb = new Q("update ZDMaxNo set NoMaxValue=? where NoType=? and NoSubType=?", new Object[] { Long.valueOf(t), noType });
  ...
  public static synchronized String getMaxNoUseLock(String noType, String subType) {
    ...
    try {
      da.setAutoCommit(false);
      Q qb = new Q("select NoMaxValue,Length from ZDMaxNo where NoType=? and NoSubType=?", new Object[] { noType, subType });
      if (Config.isOracle())
        qb.append(" for update", new Object[0]);
      ...
      for (int i = 0; i < MaxNoSet.size(); i++) {
        ...
        if ((maxno.getNoType().equals(noType)) && (maxno.getNoSubType().equals(subType)))
          synchronized (mutex) {
            ...
            return StringUtil.leftPad(String.valueOf(maxno.getNoMaxValue()), '0', (int)maxno.getLength());
          }
      }
    }
    else {
