...
public class PlatformCache extends CacheDataProvider {
	...
	public void onTypeNotFound(String type) {
		...
		if (type.equals("Role")) {
			...
			for (int i = 0; i < set.size(); i++) {
				CacheManager.set("Platform", type,
						((ZDRole) set.get(i)).getRoleCode(), set.get(i));
			}
		}
		...
		if (type.equals("Branch")) {
			...
			for (int i = 0; i < set.size(); i++) {
				CacheManager.set("Platform", type,
						((ZDBranch) set.get(i)).getBranchInnerCode(),
						set.get(i));
			}
		}
		if (type.equals("RolePriv")) {
			ZDPrivilege schema = new ZDPrivilege();
			schema.setOwnerType("R");
			 DAOSet<ZDPrivilege> set = schema.query();
	...
	public static MetaModel getMetaModel(String modelCode) {
		return (MetaModel) CacheManager.get("Platform", "MetaModel_Code",
				modelCode);
	}
	...
	public static Privilege getBranchPriv(String branchInnerCode) {
		return (Privilege) CacheManager.get("Platform", "BranchPriv",
				branchInnerCode);
	}
	...
	public static void removeRole(String roleCode) {
		CacheManager.remove("Platform", "Role", roleCode);
		 Mapx<String, Object> map = CacheManager.getMapx("Platform", "UserRole");
		synchronized (map) {
			roleCode = "," + roleCode + ",";
