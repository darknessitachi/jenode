...
public class MessageCache extends CacheDataProvider {
	...
	public static boolean addMessage(String subject, String content,
			String toUser) {
		Transaction tran = new Transaction();
		addMessage(tran, subject, content, new String[] { toUser },
				User.getUserName(), "", true);
		return tran.commit();
	}

	public static boolean addMessage(Transaction tran, String subject,
			String content, String toUser) {
		return addMessage(tran, subject, content, new String[] { toUser },
				User.getUserName(), "", true);
	}

	public static boolean addMessage(Transaction tran, String subject,
			String content, String toUser, String formUser, String RedirectURL) {
		return addMessage(tran, subject, content, new String[] { toUser },
				formUser, RedirectURL, true);
	}

	public static boolean addMessage(String subject, String content,
			String[] userList, String fromUser) {
		Transaction tran = new Transaction();
	...
	public static boolean addMessage(String subject, String content,
			String[] userList, String fromUser, String RedirectURL) {
		Transaction tran = new Transaction();
		addMessage(tran, subject, content, userList, fromUser, RedirectURL,
				true);
		return tran.commit();
	}

	public static boolean addMessage(Transaction tran, String subject,
			String content, String[] userList, String fromUser,
			String RedirectURL, final boolean popFlag) {
		ArrayList list = new ArrayList();
		for (int i = 0; i < userList.length; i++) {
			if ((!userList[i].equals(fromUser))
					&& (StringUtil.isNotEmpty(userList[i]))) {
				...
				PopMessage pm = new PopMessage(message.getID(), getHtmlMessage(
						message.getID(), message.getSubject(), content),
		...
		tran.addExecutor(new Executor(new Object[] { list }) {
			public boolean execute() {
				...
				for (int i = 0; i < list.size(); i++) {
					MessageCache.PopMessage pm = (MessageCache.PopMessage) list
							.get(i);
					MessageCache.PopMessageList pml = (MessageCache.PopMessageList) CacheManager
							.get("Message", "LastMessage", pm.ToUser);
					synchronized (pml) {
						if (popFlag) {
							pml.list.add(pm);
						}
					}
					String count = String.valueOf(CacheManager.get("Message",
							"Count", pm.ToUser));
					CacheManager.set("Message", "Count", pm.ToUser,
							Integer.valueOf(Integer.parseInt(count) + 1));
				}
				return true;
			}
		});
	...
	public static void removeIDs(DAOSet<ZDMessage> set) {
		PopMessageList pml = (PopMessageList) CacheManager.get("Message",
				"LastMessage", User.getUserName());
		int NoReadCount = 0;
		synchronized (pml) {
		...
		String count = String.valueOf(CacheManager.get("Message", "Count",
				User.getUserName()));
		CacheManager.set("Message", "Count", User.getUserName(),
				Integer.valueOf(Integer.parseInt(count) - NoReadCount));
	}
	...
	public void onKeyNotFound(String type, String key) {
		if (type.equals("Count")) {
			Q qb = new Q(
					"select count(1) from ZDMessage where ReadFlag=0 and ToUser=?",
					new Object[] { key });
	...
	public static String getHtmlMessage(long id, String subject, String content) {
		StringFormat sf = new StringFormat(
				Lang.get("Platform.Message.GetAMessage")
						+ "：<hr>?<hr>?<br><br>?");
		sf.add(subject);
		sf.add(content);
		sf.add("<p align='center' width='100%'><input type='button' class='inputButton' value='"
				+ Lang.get("Platform.MessagePopButton")
				+ "'"
				+ " onclick=\"Server.getOneValue('Message.updateReadFlag',"
				+ id + ",function(){MsgPop.closeSelf();});\"></p>");
		return sf.toString();
	}
	...
	static class PopMessageList {
		...
		public PopMessageList(String userName) {
			...
			Q qb = new Q(
					"select * from ZDMessage where ReadFlag=0 and PopFlag=0 and ToUser=? order by AddTime asc",
					new Object[] { userName });
			DataTable dt = qb.fetch();
			for (int i = 0; i < dt.getRowCount(); i++) {
				String html = MessageCache.getHtmlMessage(dt.getLong(i, "ID"),
						dt.getString(i, "Subject"), dt.getString(i, "Content"));
				this.list.add(new MessageCache.PopMessage(dt.getLong(i, "ID"),
						html, current, userName));
			}
		}
		...
		public synchronized String getLastMessage() {
			...
			for (int i = this.list.size() - 1; i >= 0; i--) {
				MessageCache.PopMessage pm = (MessageCache.PopMessage) this.list
						.get(i);
				...
				if (!pm.SessionIDMap.containsKey(User.getSessionID())) {
					...
					if (!pm.PopedFlag) {
						Q qb = new Q(
								"update ZDMessage set PopFlag=1 where ID=?",
								new Object[] { Long.valueOf(pm.ID) });
						qb.executeNoQuery();
						pm.PopedFlag = true;
					}
					return pm.Message;
