...
public class PrivBL {
	...
	public static boolean getFullPrivFlag(String type, String id) {
		...
		if ("U".equals(type)) {
			fullPrivFlag = (fullPrivFlag)
					|| (AdminUserName.getValue().equals(id));
		}
		if ("B".equals(type)) {
			fullPrivFlag = (fullPrivFlag)
					|| (PlatformCache.getBranch(id).getTreeLevel() == 1L);
		}
		return fullPrivFlag;
	}
	...
	public static boolean isInBranchPrivRange(String type, String id,
			String item) {
	...
	public static void save(RequestData request, ResponseData response,
		...
		Mapx<String, Object> map = (Mapx) request.get("Data");
		...
		DAOSet<ZDPrivilege> set = new DAOSet();
		Q wherePart;
		if (schema.fill()) {
			...
			for (String key : p2.keyArray()) {
				if ((filter.filter(key))
						|| (!isInBranchPrivRange(type, id, key))) {
					p2.remove(key);
				}
			...
			if ("B".equals(type)) {
				...
				wherePart.append(" and Owner like ?  and Owner<>?)",
						new Object[] { id + "%", id });

				wherePart
						.append("or exists (select 1 from ZDRole where BranchInnerCode like ? and ZDPrivilege.OwnerType=? and ZDPrivilege.Owner=RoleCode)",
								new Object[] { id + "%", "R" });

				wherePart
						.append("or exists (select 1 from ZDUser where BranchInnerCode like ? and ZDPrivilege.OwnerType=? and ZDPrivilege.Owner=UserName)",
								new Object[] { id + "%", "U" });
				 DAOSet<ZDPrivilege> childSet = new ZDPrivilege().query(wherePart);
		...
		if (trans.commit()) {
			for (ZDPrivilege zdDAO : set) {
				if ("R".equals(zdDAO.getOwnerType()))
					CacheManager.remove("Platform", "RolePriv",
							zdDAO.getOwner());
				else if ("B".equals(zdDAO.getOwnerType())) {
					CacheManager.remove("Platform", "BranchPriv",
							zdDAO.getOwner());
				}
			}
			response.setSuccessMessage(Lang.get("Common.SaveSuccess"));
		} else {
			response.setFailedMessage(Lang.get("Common.SaveFailed")
