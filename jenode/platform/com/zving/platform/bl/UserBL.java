...
public class UserBL
{
  ...
  public static boolean delete(Transaction trans, DataCollection dc)
  {
    ...
    for (int i = 0; i < userSet.size(); i++) {
      ...
      trans.add(new ZDPrivilege().query(new Q("where OwnerType=? and Owner=?", new Object[] { "U", user.getUserName() })), 
        5);

      DAOSet logSet = new ZDUserLog().query(new Q("where UserName=?", new Object[] { user.getUserName() }));
      trans.add(logSet, 5);
    }
    return true;
  }
  ...
  public static boolean save(Transaction trans, DataCollection dc) {
    ...
    if ((StringUtil.isNotNull(oldBranch)) && (!oldBranch.equals(user.getBranchInnerCode())) && (StringUtil.isNotNull(roleCodes))) {
      ...
      DAOSet roleSet = new ZDRole().query(new Q(" where RoleCode in ( " + code.substring(0, code.length() - 1) + ")", new Object[0]));
      roleCodes = "";
  ...
  public static Privilege getUserPriv(String userName)
  {
    ...
    if (AdminUserName.getValue().equals(userName))
    {
      ...
      for (Iterator localIterator1 = MenuPrivService.getInstance().getAll().iterator(); localIterator1.hasNext(); 
        localIterator2.hasNext())
      {
        AbstractMenuPriv priv = (AbstractMenuPriv)localIterator1.next();
        p.put(1, priv.getID());
        localIterator2 = priv.getPrivItems().keyArray().iterator(); continue; String privID = (String)localIterator2.next();
        p.put(1, privID);
      }
    }
    else {
