...
public class MetaModelBL
{
  ...
  public static boolean isNameExists(String modelName, String modelCode, String modelID)
  {
    Q qb = new Q("select count(1) from ZDMetaModel where (Name=? or Code=?)", new Object[] { modelName, modelCode });
    if (StringUtil.isNotEmpty(modelID)) {
      qb.append(" and ID<>?", new Object[] { Long.valueOf(Long.parseLong(modelID)) });
    }
  ...
  public static void insert(Mapx<String, Object> params, Transaction trans) {
    ...
    if (ObjectUtil.notEmpty(params.get("CopyID"))) {
      Q qb = new Q("where ModelID=?", new Object[] { Long.valueOf(params.getLong("CopyID")) });
