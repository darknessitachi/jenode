...
public class MetaUtil {
	...
	public static Mapx<String, Object> getExtendData(String PKValue,
			MetaModel mm) {
		Mapx map = new Mapx();
		if (mm == null) {
			return map;
		}
		Q qb = new Q("select * from " + mm.getDAO().getTargetTable()
				+ " where PKValue=? and ModelID=?", new Object[] { PKValue,
				Long.valueOf(mm.getDAO().getID()) });
		...
		for (ZDMetaColumn mc : mm.getColumns()) {
			if (mc.getDataType().equals("Datetime"))
				map.put("MetaValue_" + mc.getCode(), DateUtil
						.toDateTimeString(DateUtil.parseDateTime(metaDT
								.getString(0, mc.getTargetField()))));
			else {
				map.put("MetaValue_" + mc.getCode(),
						metaDT.get(0, mc.getTargetField()));
			}
		}
		return map;
	}
	...
	public static void attachExtendData(DataTable dt, String pkStr,
			String modelCode) {
		MetaModel mm = MetaModel.load(modelCode);
	...
	public static void attachExtendData(DataTable dt, String pkStr, MetaModel mm) {
		...
		for (int k = 0; size * k < dt.getRowCount(); k++) {
			...
			Q qb = new Q("select * from " + mm.getDAO().getTargetTable()
					+ " where ModelID=? and PKValue in ('"
					+ StringUtil.join(pkValues, "','") + "')",
					new Object[] { Long.valueOf(mm.getDAO().getID()) });
			DataTable metaDT = qb.fetch();
					...
					if (metaDT.getString(j, "PKValue").equals(pk))
						for (ZDMetaColumn mc : mm.getColumns()) {
							if (mc.getDataType().equals("Date")) {
								metaDT.getDataColumn(mc.getTargetField())
										.setDateFormat("yyyy-MM-dd");
							}
							dt.set(i + k * size, "MetaValue_" + mc.getCode(),
									metaDT.getString(j, mc.getTargetField()));
						}
			}
		}
	}
	...
	public static void deleteExtendData(Transaction tran, String PKValue,
			long modelID) {
		MetaModel mm = PlatformCache.getMetaModel(modelID);
		if (mm == null) {
			return;
		}
		Q qb = new Q("delete from " + mm.getDAO().getTargetTable()
				+ " where ModelID=? and PKValue=?", new Object[] {
				Long.valueOf(modelID), PKValue });
		tran.add(qb);
	}
	...
	public static void backupExtendData(Transaction tran, String PKValue,
			long modelID) {
		MetaModel mm = PlatformCache.getMetaModel(modelID);
		if (mm == null) {
			return;
	...
	public static void batchDeleteExtendData(Transaction tran, String PKValues,
			long modelID) {
		MetaModel mm = PlatformCache.getMetaModel(modelID);
		if (mm == null) {
			return;
		...
		Q qb = new Q("delete from " + mm.getDAO().getTargetTable()
				+ " where ModelID=? and PKValue in (" + PKValues + ")",
				new Object[] { Long.valueOf(modelID) });
		tran.add(qb);
	}

	private static String getUpdateSQL(String table, List<ZDMetaColumn> cols,
			long modelID, String pkValue) {
		...
		sb.append(" where ModelID=").append(modelID).append(" and PKValue='")
				.append(pkValue).append("'");
		return sb.toString();
	}

	private static String getInsertSQL(String table, List<ZDMetaColumn> cols,
			long modelID, String pkValue) {
		StringBuilder sb = new StringBuilder();
		...
		sb.append(") values (").append(modelID).append(",'").append(pkValue)
				.append("'");
	...
	public static void saveExtendData(Transaction tran,
			Mapx<String, Object> map, String PKValue, MetaModel mm) {
		if (mm == null) {
			return;
		}
		 Mapx<String, ZDMetaColumn> mMap = getMapping(mm.getDAO().getID());
		if (mMap == null) {
			return;
		}
		String targetTable = mm.getDAO().getTargetTable();
		Q qSelect = new Q("select count(1) from " + targetTable
				+ " where ModelID=? and PKValue=?", new Object[] {
				Long.valueOf(mm.getDAO().getID()), PKValue });
		Q q = new Q();
		if (qSelect.executeInt() > 0)
			q.setSQL(getUpdateSQL(targetTable, mMap.valueArray(), mm.getDAO()
					.getID(), PKValue));
		else {
			q.setSQL(getInsertSQL(targetTable, mMap.valueArray(), mm.getDAO()
					.getID(), PKValue));
		}
		...
		for (String key : mMap.keySet()) {
			...
			if (map.containsKey("MetaValue_" + key)) {
				...
				if (mc.getDataType().equals("Datetime")) {
					if (ObjectUtil.notEmpty(v))
						v = DateUtil.parseDateTime(map.getString("MetaValue_"
								+ key));
					else {
						v = null;
					}
				}
				if ((mc.getControlType().equals("RichText"))
						|| (mc.getControlType().equals("TextArea"))) {
	...
	public static void saveExtendData(Transaction tran,
			Mapx<String, Object> map, String PKValue, long modelID) {
		MetaModel mm = MetaModel.load(modelID);
		if (mm == null) {
			return;
		}
	...
	public static void saveExtendData(Transaction tran,
			Mapx<String, Object> map, String PKValue, String modelCode) {
		MetaModel mm = MetaModel.load(modelCode);
		if (mm == null) {
			return;
		}
	...
	public static boolean saveExtendData(Mapx<String, Object> map,
			String PKValue, long modelID) {
		Transaction tran = new Transaction();
	...
	public static boolean saveExtendData(Mapx<String, Object> map,
			String PKValue, String modelCode) {
		Transaction tran = new Transaction();
	...
	public static String getControlHTML(ZDMetaColumn mc) {
		StringBuffer sb = new StringBuffer();
		IMetadataColumnControlType controlType = MetadataColumnControlTypeServise
				.getColumnControlTypoe(mc.getControlType());
