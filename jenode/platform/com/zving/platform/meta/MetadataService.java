...
public class MetadataService extends AbstractExtendService<IMetaModelType>
{
  ...
  {
    DAOSet mms = new ZDMetaModel().query();
  ...
  public static String arrangeTargetField(long modelID, String targetField, String dataType)
    throws Exception
  {
    if (fieldMap.size() == 0) {
      loadFieldMap();
    ...
    for (int i = 0; i < mapping.size(); i++) {
      usedField[i] = ((ZDMetaColumn)mapping.valueArray().get(i)).getTargetField();
    }
