...
public class ModelTemplateService {
	public static final Pattern FieldGroupCodePattern = Pattern.compile(
			"<model:fieldgroup .*?code=\"(.*?)\">", 34);

	public static final Pattern FieldCodePattern = Pattern.compile(
			"<model:field .*?code=\"(.*?)\">", 34);
	...
	private static String parseModelFieldGroupTag(String tmpl, long modelID) {
		...
		while (tmpl.indexOf("<model:fieldgroup") > -1) {
			...
			String body = tagStr.substring(tagStr.indexOf(">") + 1,
					tagStr.indexOf("</model:fieldgroup>"));
			for (ZDMetaColumnGroup group : groups) {
				String newBody = body;
				 Mapx<String, Object> groupMap = group.toMapx();
				for (String key : groupMap.keyArray()) {
					newBody = StringUtil.replaceEx(newBody, "@{FieldGroup."
							+ key + "}", groupMap.getString(key));
				}
	...
	private static String parseModelFieldTag(String tmpl, long modelID,
			long groupID) {
		Q qb = new Q("where ModelID=? and GroupID=?", new Object[] {
				Long.valueOf(modelID), Long.valueOf(groupID) });
		...
		while (tmpl.indexOf("<model:field") > -1) {
			...
			for (ZDMetaColumn col : cols) {
				...
				for (String key : colMap.keyArray()) {
					newBody = StringUtil.replaceEx(newBody, "@{Field." + key
							+ "}", colMap.getString(key));
				}
				sb.append(newBody);
			}
	...
	public static DataTable getModelHtml(String modelCode) {
		long modelID = new Q("select ID from ZDMetaModel where Code=?",
				new Object[] { modelCode }).executeLong();
		return getModelHtml(modelID);
	}

	public static DataTable getModelHtml(long modelID) {
		DAOSet columns = new ZDMetaColumn().query(new Q("where ModelID=?",
				new Object[] { Long.valueOf(modelID) }));
		DataTable dt = columns.toDataTable();
		dt.insertColumn("ControlHtml");
		for (int i = 0; i < columns.size(); i++) {
			dt.set(i, "ControlHtml",
					MetaUtil.getControlHTML((ZDMetaColumn) columns.get(i)));
		}
		return dt;
	}
