...
public class MetaModelUI extends UIFacade {
	...
	public void bindGrid(DataGridAction dga) {
		...
		if (StringUtil.isNotEmpty(dga.getParam("SearchName"))) {
			qb.append(" and Name like ?",
					new Object[] { "%" + dga.getParam("SearchName") + "%" });
		}
  ...
  public void save() {
	...
	public void del() {
		...
		if (trans.commit()) {
			for (int i = 0; i < set.size(); i++) {
				CacheManager.remove("Platform", "MetaModel",
						((ZDMetaModel) set.get(i)).getID());
			}
	...
	public DataTable getCodeTypes() {
		Q qb = new Q(
				"select CodeType,CodeName from ZDCode where ParentCode='System'",
				new Object[0]);
	...
	private boolean isNameExists(String name, String id) {
		Q qb = new Q("select count(1) from ZDMetaModel where Name=?",
				new Object[] { name });
		if (StringUtil.isNotEmpty(id)) {
			qb.append(" and ID<>?",
					new Object[] { Long.valueOf(Long.parseLong(id)) });
		}
