...
public class DAOSet<T extends DAO<T>>
  implements Serializable, Cloneable, Iterable<T>
{
  ...
  {
    return (DAO)this.elementData.get(index);
  }
  ...
  public boolean update()
  {
    ...
    if (this.bOperateFlag) {
      ...
      for (int i = 0; i < this.operateColumnOrders.length; i++) {
        ...
        sb.append(meta.getColumns()[this.operateColumnOrders[i]].getColumnName());
        sb.append("=?");
      }
      ...
      for (DAO dao : this.elementData) {
        if (this.bOperateFlag)
          for (int i = 0; i < this.operateColumnOrders.length; i++) {
            ...
            DAOUtil.setParam(meta.getColumns()[this.operateColumnOrders[i]], pstmt, conn, i, v);
          }
        else {
          for (int i = 0; i < meta.getColumns().length; i++) {
            Object v = dao.getV(i);
            DAOUtil.setParam(meta.getColumns()[i], pstmt, conn, i, v);
          }
        }
        int i = 0;
        int i;
        for (int j = 0; i < meta.getColumns().length; i++) {
          ...
          if (sc.isPrimaryKey()) {
            ...
            DAO dao;
            if (this.bOperateFlag)
              DAOUtil.setParam(meta.getColumns()[i], pstmt, conn, j + this.operateColumnOrders.length, v);
            else {
              DAOUtil.setParam(meta.getColumns()[i], pstmt, conn, j + meta.getColumns().length, v);
            }
      ...
      for (DAO dao : this.elementData) {
        ...
        for (int j = 0; i < meta.getColumns().length; i++) {
          ...
          if (sc.isPrimaryKey()) {
            ...
            int j;
            DAO dao;
            DAOUtil.setParam(meta.getColumns()[i], pstmt, conn, j, v);
  ...
  public boolean backup(String backupOperator, String backupMemo)
  {
    ...
    try {
      ...
      DAOColumn sc2 = new DAOColumn("BackupOperator", 1, 200, 0, true, false);
  ...
  private void dealOperateColumnNames()
  {
    ...
    if (this.operateColumnNames != null) {
      ...
      int i = 0; for (int k = 0; i < this.operateColumnNames.length; i++) {
        boolean flag = false;
        for (int j = 0; j < meta.getColumns().length; j++) {
          if (this.operateColumnNames[i].toString().toLowerCase().equals(meta.getColumns()[j].getColumnName().toLowerCase())) {
        ...
        if (!flag) {
          throw new RuntimeException("Column not found:" + this.operateColumnNames[i]);
        }
      }
  ...
  public DataTable toDataTable()
  {
    ...
    if (this.bOperateFlag) {
      ...
      for (int i = 0; i < this.operateColumnOrders.length; i++) {
        ...
        dc.setColumnName(meta.getColumns()[this.operateColumnOrders[i]].getColumnName());
        dc.setColumnType(meta.getColumns()[this.operateColumnOrders[i]].getColumnType());
        dcs[i] = dc;
      }
      for (int i = 0; i < size(); i++) {
        for (int j = 0; j < this.operateColumnOrders.length; j++) {
          values[i][j] = ((DAO)this.elementData.get(i)).getV(this.operateColumnOrders[j]);
        }
      }
