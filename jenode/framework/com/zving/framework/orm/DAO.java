...
public abstract class DAO<T extends DAO<T>>
{
    ...
    try {
      ...
      for (DAOColumn sc : this.meta.getColumns()) {
        if ((sc.isMandatory()) && 
          ...
          String message = "Error:" + this.meta.getTable() + "'s column " + sc.getColumnName() + " cann't be null";
      ...
      String message = "DAO insert to " + this.meta.getTable() + " failed:" + e.getMessage();
    ...
    if (this.operateColumnFlag)
      for (int i = 0; i < this.operateColumnOrders.length; i++)
        if (isChanged(this.operateColumnOrders[i]))
        {
          ...
          String columnName = columns[this.operateColumnOrders[i]].getColumnName();
    ...
    try {
      ...
      if (this.operateColumnFlag)
        for (int i = 0; i < this.operateColumnOrders.length; i++)
          if (isChanged(this.operateColumnOrders[i]))
          {
            ...
            DAOUtil.setParam(columns[this.operateColumnOrders[i]], pstmt, conn, j++, v);
            params.add(v);
          }
      else
      ...
      for (int i = 0; i < columns.length; i++) {
        ...
        if (sc.isPrimaryKey()) {
          ...
          if (v == null) {
            String message = this.meta.getTable() + "'s primary key column " + sc.getColumnName() + " can't be null";
            DataAccess.log(start, message, null);
            LogUtil.warn(message);
            return false;
          }
      ...
      String message = "DAO update to " + this.meta.getTable() + " failed:" + e.getMessage();
      DataAccess.log(start, message, null);
      LogUtil.warn(message);
    ...
    try {
      ...
      for (int j = 0; i < columns.length; i++) {
        ...
        if (sc.isPrimaryKey()) {
          ...
          if (v == null) {
            String message = "Error:" + this.meta.getTable() + "'s primary key column " + sc.getColumnName() + " can't be null";
            DataAccess.log(start, message, null);
            LogUtil.warn(message);
            return false;
          }
      ...
      String message = "DAO delete from " + this.meta.getTable() + " failed:" + e.getMessage();
      DataAccess.log(start, message, null);
      LogUtil.warn(message);
  ...
  public String backup(String backupOperator, String backupMemo) {
    ...
    try {
      ...
      for (int i = 0; 
        ...
        if ((sc.isMandatory()) && 
          (getV(i) == null)) {
          String message = "Error:" + this.meta.getTable() + "'s column " + sc.getColumnName() + " can't be null";
          DataAccess.log(start, message, null);
          LogUtil.warn(message);
          throw new SQLException(message);
        }
      ...
      String message = "DAO backup from " + this.meta.getTable() + " failed:" + e.getMessage();
      DataAccess.log(start, message, null);
      LogUtil.warn(message);
  ...
  public boolean deleteAndBackup(String backupOperator, String backupMemo) {
    ...
    if (!this.outerConnFlag) {
      ...
      try {
        ...
        LogUtil.warn("DAO deleteAndBackup from " + this.meta.getTable() + " failed:" + e.getMessage());
        e.printStackTrace();
        this.da.rollback();
        return false;
      } finally {
  ...
  public boolean fill()
  {
    ...
    try {
      ...
      e.printStackTrace();
      String message = "DAO fill from " + this.meta.getTable() + " failed:" + e.getMessage();
      DataAccess.log(start, message, null);
      LogUtil.warn(message);
  ...
  public DAOSet<T> fetch(QueryBuilder wherePart, int pageSize, int pageIndex) {
    ...
    DAOColumn[] columns = this.meta.getColumns();
    if ((wherePart != null) && 
      (!wherePart.getSQL().trim().toLowerCase().startsWith("where"))) {
    ...
    try {
      if (this.operateColumnFlag) {
        for (int i = 0; i < this.operateColumnOrders.length; i++) {
          ...
          sb.append(columns[this.operateColumnOrders[i]].getColumnName());
        }
      }
      ...
      String message = "DAO query from " + this.meta.getTable() + " failed:" + e.getMessage();
  ...
  private void setVAll(DBConn conn, DAO<?> dao, ResultSet rs) throws Exception {
    ...
    if (this.operateColumnFlag) {
      for (int i = 0; i < this.operateColumnOrders.length; i++) {
        ...
        if ((columns[order].getColumnType() == 10) || (columns[order].getColumnType() == 1)) {
      ...
      for (int i = 0; i < columns.length; i++) {
        ...
        if ((columns[i].getColumnType() == 10) || (columns[i].getColumnType() == 1)) {
  ...
  public void setOperateColumns(String[] colNames) {
    ...
    int i = 0; for (int k = 0; i < colNames.length; i++) {
      ...
      if (!flag) {
        throw new DAOException("Column not found in table " + this.meta.getTable() + ":" + colNames[i]);
      }
    }
  ...
  public void setV(int i, Object v)
  {
    ...
    if ((i < 0) || (i >= this.meta.getColumns().length)) {
      throw new DAOException("DAO.setV() failed,index out of range:" + this.meta.getTable());
    }
  ...
  {
    try {
      return (DAO)getClass().newInstance();
    } catch (Exception e) {
