...
public class ZhtmlManager implements ITemplateManager {
	...
	public boolean execute(String url, AbstractExecuteContext context)
		...
		if ((context instanceof HttpExecuteContext)) {
			HttpServletRequest request = ((HttpExecuteContext) context)
					.getRequest();
	...
	private static boolean needReload(TemplateExecutor je, String fileName)
		...
		if (System.currentTimeMillis() - je.getLastCheckTime() > 3000L) {
			File f = new File(FileUtil.normalizePath(Config
					.getContextRealPath() + "/" + fileName));
			if (!f.exists()) {
				map.remove(fileName);
	...
	public TemplateExecutor getExecutor(String fileName)
			throws TemplateCompileException {
		TemplateExecutor je = (TemplateExecutor) map.get(fileName);
		try {
			if ((je == null) || (needReload(je, fileName))) {
				TemplateCompiler jc = new TemplateCompiler(
						ZhtmlManagerContext.getInstance());
