...
public class BuiltResourceScanner {
	...
	public static void load() {
		if ((lastTime == 0L)
				|| ((Config.isDebugMode()) && (System.currentTimeMillis()
						- lastTime > 3000L)))
			synchronized (mutex) {
				if ((lastTime == 0L)
						|| ((Config.isDebugMode()) && (System
								.currentTimeMillis() - lastTime > 3000L))) {
					scanAll(BuiltResourceVisitorService.getInstance().getAll(),
							null);
					ConfigLoader.reload();
	...
	public static void scanAll(List<? extends IBuiltResourceVisitor> visitors,
			String webinfPath) {
		...
		if (webinfPath == null) {
		 ...
		 ArrayList<PluginConfig> list = pm.getAllPluginConfig();
		 
		 for (PluginConfig pc : list) {
			 scanJar(new File(pluginPath + "lib/" + pc.getID() + ".plugin.jar"),
						visitors);
					...
					if (path.startsWith("[D]")) {
						...
						if (path.startsWith("JAVA")) {
							...
							try {
								...
								e.printStackTrace();
								LogUtil.error("Load class directory failed:"
										+ e.getMessage());
							}
						}
		...
		if (lastTime == 0L) {
			LogUtil.info("----" + Config.getAppCode() + "("
					+ LangUtil.get(Config.getAppName())
					+ "): Scan class and resource used "
					+ (System.currentTimeMillis() - t) + " ms----");
		}
		lastTime = System.currentTimeMillis();
	}

	private static void scanJar(File f,
			List<? extends IBuiltResourceVisitor> visitors) {
		try {
			if (!f.exists()) {
				return;
			}
	...
	private static void scanOneDir(File p, String prefix,
			List<? extends IBuiltResourceVisitor> visitors) throws Exception {
		if (!p.exists()) {
			return;
		}
		...
		for (int i = 0; i < fs.length; i++) {
			...
			if (f.isFile()) {
				...
				scanOneDir(f, prefix, visitors);
			}
		}
	}

	public static void scanOneResource(BuiltResource br,
			List<? extends IBuiltResourceVisitor> vistors) throws Exception {
		if (br.getFullName().indexOf("com/zving/framework/") >= 0) {
			return;
		}
		ClassNode cn = null;
		for (IBuiltResourceVisitor cv : vistors)
			if ((cv.match(br)) && (cn == null))
				if (br.isClass()) {
