...
public class DispatchServlet extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		if (Config.ServletMajorVersion == 0) {
			...
			Config.getMapx().put("System.ContainerInfo",
			...
			List<XMLElement> list = p.parse().elements(
	...
	public void forwardToDefaultServlet(HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {
		RequestDispatcher rd = context
				.getNamedDispatcher(this.defaultServletName);
		if (rd == null) {
			throw new IllegalStateException(
	...
	public void service(ServletRequest req, ServletResponse rep)
		...
		String url = requestURI
				.substring(context.length(), requestURI.length());
		url = url.replaceAll("/+", "/");
		...
		try {
			...
			if (Thread.currentThread().getContextClassLoader() != PreClassLoader
					.getInstance()) {
				httpThreads.put(Thread.currentThread(), Thread.currentThread()
			...
			if (!"true".equals(request.getParameter("_ZVING_NOSESSION"))) {
				...
				if (session != null) {
					User.UserData u = SessionListener
					...
					if (HttpSessionListenerFacade.getSession(session.getId()) == null) {
						HttpSessionListenerFacade.setSession(session.getId(),
								session);
					}
					User.setCurrent(u);
				} else {
			...
			for (IExceptionCatcher ec : ExceptionCatcherService.getInstance()
					.getAll()) {
				Class[] arrayOfClass;
				j = (arrayOfClass = ec.getTargetExceptionClass()).length;
				i = 0;
				for (Class c : arrayOfClass) {
					if (c.isInstance(e)) {
	...
	private void handleURL(String url, HttpServletRequest request,
				...
				if (f.exists()) {
					if (f.isFile()) {
						...
						for (String str : this.welcomeFiles)
							for (IURLHandler up : URLHandlerService
									.getInstance().getAll())
								if (up.match(url + str)) {
									Current.setURLHandler(up);
	...
	public void destroy() {
		try {
			...
			for (Thread t : httpThreads.keyArray())
				if (t.isAlive())
					try {
						t.setContextClassLoader((ClassLoader) httpThreads
								.get(t));
