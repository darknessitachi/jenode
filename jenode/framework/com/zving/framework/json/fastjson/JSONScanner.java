  ...
  implements JSONLexer
{
  ...
  public final String scanSymbolUnQuoted(SymbolTable symbolTable) {
    ...
    boolean firstFlag = (this.ch >= firstIdentifierFlags.length) || (firstIdentifierFlags[first] != 0);
    if (!firstFlag) {
  ...
  public String scanFieldSymbol(char[] fieldName, SymbolTable symbolTable) {
    ...
    do {
      ...
      if (ch == '"') {
        this.bp = index;
        this.ch = (ch = this.buf[this.bp]);
        String strVal = symbolTable.addSymbol(this.buf, start, index - start - 1, hash);
        break;
      }
    ...
    throws NumberFormatException
  {
    ...
    int i = this.np; int max = this.np + this.sp;
    long limit;
    if (this.buf[this.np] == '-') {
      negative = true;
      long limit = -9223372036854775808L;
      i++;
    } else {
