...
public class MenuManager {
	...
	public static Mapx<String, Menu> getMenus() {
		synchronized (mutex) {
			...
			if ((Config.isDebugMode())
					&& (System.currentTimeMillis() - lastTime > 5000L)) {
				...
				if (new File(path + "/lib/").exists()) {
					...
					for (File f : fs) {
						if (f.getName().endsWith(".plugin.jar")) {
							try {
								 Mapx<String, Long> files = ZipUtil.getFileListInZip(f
										.getAbsolutePath());
								for (String fileName : files.keyArray()) {
									if (fileName.endsWith(".plugin")) {
										byte[] bs = ZipUtil.readFileInZip(
												f.getAbsolutePath(), fileName);
										PluginConfig pluginConfig = new PluginConfig();
										pluginConfig.parse(new String(bs,
												"UTF-8"));

										pc = PluginManager.getInstance()
												.getPluginConfig(
														pluginConfig.getID());
										if ((pc == null) || (pc.isEnabling()))
											for (Menu m : pluginConfig
													.getMenus().values())
												menus.put(m.getID(), m);
									}
								}
				...
				dir = Config.getContextRealPath()
						+ "WEB-INF/plugins/classes/plugins/";
				File p = new File(dir);
						...
						if (f.getName().endsWith(".plugin")) {
							...
							try {
								pluginConfig.parse(xml);
								 ...
								 pc = PluginManager.getInstance()
										.getPluginConfig(pluginConfig.getID());
								if ((pc == null) || (pc.isEnabling()))
									for (Menu m : pluginConfig.getMenus()
											.values())
										menus.put(m.getID(), m);
			...
			if (menus == null) {
				...
				for (PluginConfig pc : PluginManager.getInstance()
						.getAllPluginConfig())
					if (pc.isEnabling()) {
						for (menuCfg = pc.getMenus().values().iterator(); ((Iterator) menuCfg)
								.hasNext();) {
							Menu m = (Menu) ((Iterator) menuCfg).next();
							menus.put(m.getID(), m);
						}
					}
			}
	...
	private static void writeMenuCfgFile() {
		synchronized (mutex) {
			...
			String menuCfg = Config.getContextRealPath()
					+ "WEB-INF/plugins/classes/plugins/menu.config";
			if (!FileUtil
					.exists(menuCfg.substring(0, menuCfg.lastIndexOf("/")))) {
	...
	public static boolean deleteMenu(String mid) {
		if (ObjectUtil.empty(menus.get(mid))) {
	...
	public static boolean delMenu(Menu m) {
		...
		for (String mID : menus.keyArray()) {
			...
			if ((menu.getParentID() != null) && (menu.getParentID().equals(ID))) {
				PluginConfig pc = PluginManager.getInstance().getPluginConfig(
