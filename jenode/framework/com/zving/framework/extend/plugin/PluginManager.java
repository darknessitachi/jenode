...
public class PluginManager {
	...
	public ArrayList<PluginConfig> loadAllConfig(String path) {
		...
		File statusFile = new File(path
				+ "/plugins/classes/plugins/status.config");
		if (statusFile.exists()) {
			this.statusMap = PropertiesUtil.read(statusFile);
		}
		InputStream is = PluginManager.class
				.getResourceAsStream("/plugins/com.zving.framework.plugin");
		try {
			if (is != null) {
	...
	private void loadFromLib(String path, ArrayList<PluginConfig> list,
			Mapx<String, String> map) {
						...
						for (String fileName : files.keyArray())
							if (fileName.endsWith(".plugin")) {
								...
								if (!map.containsKey(pc.getID()))
									...
									LogUtil.warn("PluginConfig is duplication:"
											+ (String) map.get(pc.getID())
											+ " & " + f.getAbsolutePath() + "!"
											+ fileName);
								}
	...
	private void loadFromClasses(String path, ArrayList<PluginConfig> list,
			Mapx<String, String> map) {
	...
	private void sort(ArrayList<PluginConfig> result,
			ArrayList<PluginConfig> list, PluginConfig pc) {
			...
			if (pc.isEnabling()) {
				...
				for (String pluginID : pc.getRequiredPlugins().keySet()) {
					...
					if ((c == null) || (!c.isEnabling())) {
						if (c == null) {
							LogUtil.error("Plugin " + pluginID + " needed by "
									+ pc.getID() + " is not found!");
						}
						requiredFlag = false;
						break;
					}
					...
					if (!isVersionCompatible(need, v)) {
						LogUtil.error("Plugin " + pluginID + "'s version is "
								+ v + ", but " + need + " is needed by "
								+ pc.getID() + "!");
					}
				}
				if (!requiredFlag) {
					...
					requiredFlag = true;
					for (String extendPointID : pc.getRequiredExtendPoints()
							.keyArray()) {
						boolean flag = false;
						...
						if (!flag) {
							LogUtil.error("ExtendPoint " + extendPointID
									+ " needed by " + pc.getID()
									+ " is not found!");
							requiredFlag = false;
							break;
