...
public class ExtendManager {
	...
	public void start() {
		synchronized (this.mutex) {
			if (this.extendActionMap == null) {
				...
				ArrayList<PluginConfig> configList = PluginManager.getInstance()
						.getAllPluginConfig();
				...
				LogUtil.info("All plugins started,cost "
						+ (System.currentTimeMillis() - t) + " ms");
				invoke("com.zving.framework.AfterAllPluginStarted",
						new Object[0]);
				invoke("com.zving.framework.AfterPluginInitAction",
						new Object[0]);
			}
		}
	}
			...
			synchronized (this.mutex) {
				try {
					...
					for (String id : pc.getRequiredPlugins().keyArray()) {
						startPlugin(PluginManager.getInstance()
								.getPluginConfig(id));
					}
					Class c = Class.forName(pc.getClassName());
					if (!IPlugin.class.isAssignableFrom(c)) {
						LogUtil.error("Plugin class '" + pc.getClassName()
								+ "' isn't inherit from IPlugin");
						return;
					}

					this.extendPointMap.putAll(pc.getExtendPoints());

					for (ExtendServiceConfig es : pc.getExtendServices()
							.valueArray()) {
					...
					for (ExtendActionConfig action : actions) {
						if (!this.extendPointMap.containsKey(action
								.getExtendPointID())) {
							LogUtil.error("ExtendAction " + action.getID()
									+ "'s ExtendPoint not found");
						} else {
							list = (ArrayList) this.extendActionMap.get(action
									.getExtendPointID());
							if (list == null) {
								list = new ArrayList();
								this.extendActionMap.put(
										action.getExtendPointID(), list);
							}
							list.add(action);
						}
					}
					...
					for (ExtendItemConfig item : items) {
						if (!this.extendServiceMap.containsKey(item
								.getExtendServiceID())) {
							LogUtil.error("ExtendItem " + item.getID()
									+ "'s ExtendService not found");
						} else {
							es = (ExtendServiceConfig) this.extendServiceMap
									.get(item.getExtendServiceID());
							try {
							...
							list = (ArrayList) this.extendItemMap.get(item
									.getExtendServiceID());
							if (list == null) {
								list = new ArrayList();
								this.extendItemMap.put(
										item.getExtendServiceID(), list);
							}
							list.add(item);
						}
					}

					Collection<ReplaceableItemConfig> rs = pc.getReplaceableItems().values();
					for (ReplaceableItemConfig r : rs) {
						this.replaceableItemMap.put(r.getID(), r);
					}

					Collection<OptionalItemConfig> os = pc.getOptionalItems().values();
					for (OptionalItemConfig oic : os) {
						 list = (ArrayList) this.optionalItemMap
								.get(oic.getTargetID());
						if (list == null) {
							list = new ArrayList();
	...
	public List<PluginConfig> getRequiredPlugins(PluginConfig pc)
		...
		for (PluginConfig pc2 : PluginManager.getInstance()
				.getAllPluginConfig()) {
			if ((!pc2.getID().equals(pc.getID()))
					&& (pc2.getRequiredPlugins().containsKey(pc.getID()))) {
				list.add(pc2);
			}
		}
		return list;
		...
		if (ObjectUtil.notEmpty(pc.getClassName()))
			synchronized (this.mutex) {
				try {
					...
					for (ExtendActionConfig ea : pc.getExtendActions()
							.valueArray()) {
						((ArrayList) this.extendActionMap.get(ea
								.getExtendPointID())).remove(ea);
					}
					...
					for (ExtendItemConfig ei : pc.getExtendItems().valueArray()) {
						ExtendServiceConfig es = (ExtendServiceConfig) this.extendServiceMap
								.get(ei.getExtendServiceID());
						if (es != null) {
							es.getInstance().remove(ei.getInstance().getID());
						}
						((ArrayList) this.extendItemMap.get(ei
								.getExtendServiceID())).remove(ei);
					}
	...
	public ArrayList<ExtendActionConfig> findActionsByPointID(
			String extendPointID) {
	...
	public Object[] invokePoint(String extendPointID, Object[] args) {
		try {
			...
			for (int i = 0; i < actions.size(); i++) {
				try {
					IExtendAction ea = ((ExtendActionConfig) actions.get(i))
							.getInstance();
					if (ea.isUsable()) {
	...
	public boolean isPluginEnable(String pluginID) {
		return !"false".equals(PluginManager.getInstance().getStatusMap()
				.get(pluginID));
	}
	...
	private void setStatusValue(String key, String value) {
		File f = new File(Config.getPluginPath()
				+ "classes/plugins/status.config");
