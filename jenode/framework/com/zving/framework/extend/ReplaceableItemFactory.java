...
public class ReplaceableItemFactory
{
    ...
    if (singletonMap == null)
      synchronized (ReplaceableItemFactory.class) {
        if (singletonMap == null) {
          ...
          for (Iterator localIterator1 = PluginManager.getInstance().getAllPluginConfig().iterator(); localIterator1.hasNext(); 
            localIterator2.hasNext())
          {
            PluginConfig pc = (PluginConfig)localIterator1.next();
            localIterator2 = pc.getReplaceableItems().keyArray().iterator(); continue; String id = (String)localIterator2.next();
            ReplaceableItemConfig ric = (ReplaceableItemConfig)pc.getReplaceableItems().get(id);
  ...
  {
    init();
    return get((String)classnameMap.get(clazz.getName()));
  }

  public static Object get(String id) {
    init();
    ReplaceableItemConfig ric = ExtendManager.getInstance().findReplaceableItem(id);
    return get(ric);
  }
}